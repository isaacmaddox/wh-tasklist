...
export function ListTasks() {
  ...
  return (
    <ul className="grid grid-cols-[max-content_1fr_max-content_max-content_max-content] gap-3 gap-y-4">
...
function TaskItem({ defaultTask, taskId }: TaskItemProps) {
  ...
  return (
    <li
      ...
      <Checkbox id={`task-${taskId}`} checked={task.completed} className="self-start mt-2.5" onCheckedChange={toggleTaskCompleted} />
      {!isEditing && (
        <>
          <label
            htmlFor={`task-${taskId}`}
            className={cn("block min-h-9 leading-9", isOverdue && "text-destructive", task.completed && "text-muted-foreground")}>
            {task.name}
          </label>
          <label htmlFor={`task-${taskId}`} className={cn(isOverdue && "text-destructive", task.completed && "text-muted-foreground")}>
