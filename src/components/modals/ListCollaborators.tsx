          ...
          {canUserModifyList && (
            <>
              ...
              <ul className="grid grid-cols-[1fr_max-content_max-content] gap-3">
                {groupedShares["false"]?.map(([email]) => {
                  return (
                    <li key={email} className="grid grid-cols-subgrid col-span-full items-center">
                      ...
                      {list.owner_id === user.uid && (
                        <>
                          <Button size="icon-xs" variant="ghost" onClick={() => doRemoveCollaborator(email, true)}>
                            <XIcon />
                          </Button>
                ...
                {!groupedShares["false"] && <li className="col-span-full text-muted-foreground">No past collaborators</li>}
              </ul>
            </>
          )}
        </div>
